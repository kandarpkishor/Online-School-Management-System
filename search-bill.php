...
<div class="search_panel">
		<form action="" method="post">
			...
			<input style="width:120px; color:blue; font-size:15px;"type="submit" name="findbill" value="Find Bill"/></br>
		</form>
	</div>
<?php
	...
	{	
		...
		else
		{
			...
			$month = $_POST['month'];
			$year = $_POST['year'];
			$query1 = mysqli_query($conn,"SELECT *FROM bill WHERE CLASS='$class' AND MONTH= '$month' AND YEAR = '$year'");
		...
		echo '<tr><th>ID</th><th>BILL NO.</th><th>NAME</th><th>CLASS</th><th>ROLL</th><th>MONTH</th><th>YEAR</th><th>BILL AMOUNT</th><th>BILL DATE</th><th>ACTION</th></tr>';
			...
			echo '<tr><td>'.$sl_no.'</td><td>'.$row['BILL_ID'].'</td><td>'.ucwords(strtolower($row['S_NAME'])).'</td><td>'.$row['CLASS'].'</td><td>'.$row['ROLL'].'</td><td>'.$row['MONTH'].
			'</td><td>'.$row['YEAR'].'</td><td>'.$bill_amount.'</td><td>'.$row['DATE'].'</td>';
			echo '<td><a target="_blank" href="bill.php?id='.$row['BILL_ID'].'"> <input type="button" name="prnt" value="Print"/></a></td></tr>';
				}
				echo '</table>';
			...
			{
				echo '<span class="warning">Bill not found for Class '.$class.' For the Month of '.$month.'/'.$year.'</span>';
			}
		}
	}
	else
	{	
		...
		$query2 = mysqli_query($conn,"SELECT *FROM bill LIMIT $search_from, 20");
		//$row = mysqli_fetch_array($query1,MYSQLI_BOTH);
		$sl_no=0;
		echo '<table class="table" style="width:1000px;">';
		echo '<tr><th>ID</th><th>BILL NO.</th><th>NAME</th><th>CLASS</th><th>ROLL</th><th>MONTH</th><th>YEAR</th><th>BILL AMOUNT</th><th>BILL DATE</th><th>ACTION</th></tr>';
		...
		echo '</table>';
			...
			$result = mysqli_query($conn,"SELECT COUNT(ID) AS total FROM bill");
