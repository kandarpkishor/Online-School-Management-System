...
<!DOCTYPE html>
 <head>
  ...
  <body>
   ...
   {
	   $massage=$_POST['smsMessage'];
	   $phone=$_POST['phoneNumber'];
	   if(empty($massage) || empty($phone))
	   {
		   echo '<script> alert("Please Type Phone Number and Massage")</script>';
	   }
	   else
		   
	   {
		   $pass='gOlU@4658';
		   $number=array('8877972243','8789575006','6201851792');
		  $str = implode (",", $number);
		  
		   echo $str;
		 //echo extract($number, EXTR_PREFIX_SAME, "dup");
		  header('location:http://www.smsidea.co.in/sendsms.aspx?mobile=8298444957&pass='.$pass.'&senderid=SMSBUZ&to='.$phone.'&msg='.$massage.'');
