...
<?php
	...
	{	
		...
		{
			$billid = $_POST['billid'];
			$query1 = mysqli_query($conn,"SELECT *FROM payment WHERE (BILL_ID='$billid' OR S_ID='$billid' OR S_NAME='$billid') AND CLASS='$class' AND MONTH='$month' AND YEAR='$year' AND DUES>00");
	...
	{	
		...
		{
			$sid=$row1['S_ID'];
			$query3 = mysqli_query($conn,"SELECT *FROM payment WHERE S_ID='$sid' AND CLASS='$class' AND MONTH='$month' AND YEAR='$year' AND DUES>0 ORDER BY ID DESC LIMIT $search_from, 20");
